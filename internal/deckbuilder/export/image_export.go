// Package deckexport creates deckbuilder-specific Tabletop Simulator assets.
package deckexport
...
func WriteDeckImage(
	...
	if len(mainCardIDs) > decks.MaxMainDeckCards {
		return fmt.Errorf(
...
func WriteSideboardImage(
	...
	if len(sideCardIDs) > decks.MaxSideDeckCards {
		return fmt.Errorf(
...
func ExportDeckImage(path string, deck *decks.Deck) (err error) {
...
func ExportSideboardImage(path string, deck *decks.Deck) (err error) {
...
func exportDeckCardSheet(
	path string,
	deck *decks.Deck,
	writeImage func(io.Writer, *decks.Deck, string) error,
) (err error) {
...
func cardIDsForImageExport(entries []decks.DeckEntry, order []string) []string {
...
// totalCards returns the physical card count represented by aggregate entries.
func totalCards(entries []decks.DeckEntry) int {
	total := 0
	for _, entry := range entries {
