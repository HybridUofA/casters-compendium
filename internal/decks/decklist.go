...
func ReadDeckList(reader io.Reader, repository *cards.Repository) (*Deck, error) {
	if reader == nil {
	...
	deckName := ""
	zone := Zone("")
	declaredTotals := make(map[Zone]int)
	seenZones := make(map[Zone]bool)
	deck := &Deck{SchemaVersion: 1}
	...
	if !seenZones[MainZone] || !seenZones[SideZone] {
		return nil, fmt.Errorf("decklist must contain main and side deck sections")
	}
	deck.Name = deckName
	if deck.MainTotal() != declaredTotals[MainZone] {
		return nil, fmt.Errorf(
			"main deck declares %d cards but contains %d",
			declaredTotals[MainZone],
			deck.MainTotal(),
		)
	}
	if deck.SideTotal() != declaredTotals[SideZone] {
		return nil, fmt.Errorf(
			"side deck declares %d cards but contains %d",
			declaredTotals[SideZone],
			deck.SideTotal(),
		)
	}
	deck.EnsureOrder()
